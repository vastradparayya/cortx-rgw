...
{
  int ret;
  string tenant_bucket = info.bucket.tenant;
  if(tenant_bucket != ""){
    tenant_bucket += "$"+info.bucket.name;
  }else{
    tenant_bucket = info.bucket.name;
  }

  ldpp_dout(dpp, 20) << "remove_bucket Entry=" << tenant_bucket << dendl;
  ...
  // 3. Remove mp index??
  string bucket_multipart_iname = "motr.rgw.bucket." + tenant_bucket + ".multiparts";
  ...
  string bucket_index_iname = "motr.rgw.bucket.index." + tenant_bucket;
  ...
  bufferlist bl;
  ret = store->get_bucket_inst_cache()->remove(dpp, tenant_bucket);
  if (ret < 0) {
  ...
  ldpp_dout(dpp, 20) << "remove_bucket Exit=" << tenant_bucket << dendl;
...
int MotrBucket::put_info(const DoutPrefixProvider *dpp, bool exclusive, ceph::real_time _mtime)
{
  ...
  int rc = store->do_idx_op_by_name(RGW_MOTR_BUCKET_INST_IDX_NAME,
                                  M0_IC_PUT, tenant_bucket, bl, !exclusive);
  if (rc == 0)
    store->get_bucket_inst_cache()->put(dpp, tenant_bucket, bl);
...
int MotrBucket::load_bucket(const DoutPrefixProvider *dpp, optional_yield y, bool get_stats)
{
  ...
  if (store->get_bucket_inst_cache()->get(dpp, tenant_bucket, bl)) {
    // Cache misses.
    ldpp_dout(dpp, 20) << "load_bucket(): name=" << tenant_bucket << dendl;
    int rc = store->do_idx_op_by_name(RGW_MOTR_BUCKET_INST_IDX_NAME,
                                      M0_IC_GET, tenant_bucket, bl);
...
int MotrBucket::list(const DoutPrefixProvider *dpp, ListParams& params, int max, ListResults& results, optional_yield y)
{
  ...
  ldpp_dout(dpp, 20) << "bucket=" << tenant_bucket
      ...
      bool *is_truncated)
{
  ...
  string tenant_bucket = this->get_tenant();
  if(tenant_bucket != ""){
    tenant_bucket += "$"+this->get_name();
  }else{
    tenant_bucket = this->get_name();
  }

  string bucket_multipart_iname =
      "motr.rgw.bucket." + tenant_bucket + ".multiparts";
  key_vec[0].clear();
...
int MotrObject::get_obj_state(const DoutPrefixProvider* dpp, RGWObjectCtx* rctx, RGWObjState **_state, optional_yield y, bool follow_olh)
{
  ...
  *_state = state;
  ...
  bufferlist bl;
  string tenant_bucket = this->get_bucket()->get_tenant();
  if(tenant_bucket != ""){
    tenant_bucket += "$"+this->get_bucket()->get_name();
  }else{
    tenant_bucket = this->get_bucket()->get_name();
  }
...
int MotrObject::get_obj_attrs(RGWObjectCtx* rctx, optional_yield y, const DoutPrefixProvider* dpp, rgw_obj* target_obj)
{
  ...
  string bname, key;
  if (target_obj) {
    bname = target_obj->bucket.tenant;
    if(bname != ""){
    ...
    bname = this->get_bucket()->get_tenant();
    if(bname != ""){
...
{
  string tenant_bucket = source->get_bucket()->get_tenant();

  if(tenant_bucket != ""){
    tenant_bucket += "$"+source->get_bucket()->get_name();
  }else{
    tenant_bucket = source->get_bucket()->get_name();
  }
  ldpp_dout(dpp, 20) << "delete " << source->get_key().to_str() << " from " << tenant_bucket << dendl;
                       ...
                       optional_yield y)
{
  ...
  string tenant_bucket = obj.get_bucket()->get_tenant();
  if(tenant_bucket != ""){
    tenant_bucket += "$"+obj.get_bucket()->get_name();
  }else{
    tenant_bucket = obj.get_bucket()->get_name();
  }
...
int MotrMultipartUpload::delete_parts(const DoutPrefixProvider *dpp)
{
  ...
  string tenant_bucket = bucket->get_tenant();
  if(tenant_bucket != ""){
    tenant_bucket += "$"+bucket->get_name();
  }else{
    tenant_bucket = bucket->get_name();
  }
  string obj_part_iname = "motr.rgw.object." + tenant_bucket + "." + oid + ".parts";
                                ...
                                RGWObjectCtx *obj_ctx)
{
  ...
  string tenant_bucket = meta_obj->get_bucket()->get_tenant();
  if(tenant_bucket != ""){
    tenant_bucket += "$"+meta_obj->get_bucket()->get_name();
  }else{
    tenant_bucket = meta_obj->get_bucket()->get_name();
  }
  string bucket_multipart_iname =
                       ...
                       optional_yield y)
{
  ...
  p.append(buf);
  string tenant_bucket = head_obj->get_bucket()->get_tenant();
  if(tenant_bucket != ""){
    tenant_bucket += "$"+head_obj->get_bucket()->get_name();
  }else{
    tenant_bucket = head_obj->get_bucket()->get_name();
  }
  string obj_part_iname = "motr.rgw.object." + tenant_bucket + "." +
